        ...
        public async Task<List<VentaDTO>> Historial(string buscarPor, string numeroVenta, string fechaInicio, string fechaFin)
        {
            ...
            try
            {
                if (buscarPor == "fecha")
                {
                    ...
                    DateTime fecha_Fin = DateTime.ParseExact(fechaInicio, "dd/MM/yyyy", new CultureInfo("es-AR"));

                    ListaResultado = await query.Where(v =>
                        v.FechaRegistro.Value >= fecha_inicio.Date &&
                        v.FechaRegistro.Value <= fecha_Fin.Date
